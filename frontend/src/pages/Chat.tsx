...
import { IoMdSend, IoMdMic, IoMdImage } from "react-icons/io"; // Import IoMdImage
...
const Chat = () => {
  ...
  const auth = UserAuth();
  const [chatMessages, setChatMessages] = useState<{ role: string; content: string; imageUrl?: string }[]>([]); // Updated type definition
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    let imageUrl: string | null = null;  // Store the image URL
    ...
    try {
      ...
      if (data.text) {
        // Get the image URL
        imageUrl = URL.createObjectURL(file); // Create a URL from the file

        setChatMessages((prev) => [
          ...prev,
          { role: "user", content: "", imageUrl }, // Send image as user
          { role: "assistant", content: data.text }, // Assistant's response
        ]);
      }
    } catch (error) {
      ...
      <Box sx={{ display: "flex", flex: { md: 0.8, xs: 1, sm: 1 }, flexDirection: "column", px: 3 }}>
        ...
        >
          {chatMessages.map((chat, index) => (
            <ChatItem content={chat.content} role={chat.role} imageUrl={chat.imageUrl} key={index} />
          ))}
        </Box>
        <div
          style={{
